...
DEFAULT_SPREAD_BPS   = 5.0   # half-spread per side, bps — this is the spread
                              # for a LIQUID reference name; scaled wider for
                              # illiquid symbols, see _liquidity_scaled_spread_bps
DEFAULT_IMPACT_FACTOR = 0.1  # market impact scaling coefficient

# L4 FIX: dollar ADV of a "liquid" large-cap reference name (e.g. AAPL-tier).
# Spread widens as a symbol's actual ADV falls below this. Previously
# spread_bps was a single flat constant applied to every symbol regardless
# of liquidity — market impact already scaled with participation rate, but
# the spread half didn't, so illiquid/small-float names (meme-stock-tier
# tickers with real-world spreads of 20-100+ bps) were modeled with the
# same ~5bps spread as a mega-cap, systematically understating their real
# transaction costs and overstating backtested returns on those symbols.
REFERENCE_LIQUID_ADV  = 1e9
MAX_SPREAD_SCALE      = 10.0  # cap widening at 10x the base spread
...
class FillModel:
    ...
    def _liquidity_scaled_spread_bps(self, adv: float) -> float:
        """
        L4 FIX: widen the assumed half-spread for illiquid symbols instead
        of applying self.spread_bps flat to everything. sqrt-dampened so a
        100x drop in ADV doesn't naively imply 100x wider spreads, and
        capped at MAX_SPREAD_SCALE so a near-zero ADV bar can't blow the
        cost model up to something absurd.
        """
        if adv <= 0:
            return self.spread_bps * MAX_SPREAD_SCALE
        scale = max(1.0, (REFERENCE_LIQUID_ADV / adv) ** 0.5)
        scale = min(scale, MAX_SPREAD_SCALE)
        return self.spread_bps * scale
    ...
    def simulate_fill(
        ...
        # Dollar ADV proxy, computed first so both spread and impact scale
        # with actual liquidity (previously only impact did — see L4 FIX).
        adv = float(bar.get('volume', 1e6)) * arrival   # dollar volume proxy

        # Half-spread cost (bps → price) — widened for illiquid symbols
        effective_spread_bps = self._liquidity_scaled_spread_bps(adv)
        spread_dollars = arrival * (effective_spread_bps / 1e4)
